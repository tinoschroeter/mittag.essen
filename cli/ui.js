...
const { Rezept, Zutaten, Arbeitszeit, Backzeit, Gesamtzeit, Zubereitung } =
  data[key];

const preparationBoxLenght =
  Zubereitung.split("\n")
    .map((a) => a.length)
    .sort((a, b) => b - a)[0] + 5;
    ...
    <Box width={150}>
      ...
      <Box flexBasis="20%">
        <Box borderStyle="classic">
          <Gradient name="retro">
            <Text>
              Kochzeit: {Backzeit ? `${Backzeit} ` : `${Arbeitszeit} `}
            </Text>
          </Gradient>
        </Box>
      </Box>
